...
def write_jobs(sku='G1', target_service='sing', target_name='msroctows', sla_tier='premium',
               model_name='baseline_vit_base_in21k_224', datasets=['aircraft', 'car', 'cub', 'dog', 'web_aircraft', 'web_bird', 'web_car']):
    ...
    with open(f'./amlt_config/' + yaml_name + '.yaml', 'w', encoding='utf-8') as w:
        ...
        config_files = os.listdir(os.path.join('configs', model_name))
        jobs_list = []
        for file in config_files:

            # only select one seed for tunning
            if not file.endswith('seed42.yaml'):
                continue
            ...
            job_name = f'{model_name}_{job_name}_tune'
            command = f"python tune.py --config {config_path} --n_cpu 4 --n_gpu 1 --n_trials 30"
