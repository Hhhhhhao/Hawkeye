    ...
    def __init__(self, 
        ...
        * num_selects:
        ...
        if return_nodes is not None:
            ...
            self.backbone = Inspect(backbone, return_nodes)
        else:
            self.backbone = backbone
        
        ### get hidden feartues size
        rand_in = torch.randn(1, 3, img_size, img_size)
        _, outs = self.backbone(rand_in)
...
def PIM(config):
    ...
    elif name in ['vit_small_patch16_224', 'vit_base_patch16_224', 'vit_small_patch16_224_in21k', 'vit_base_patch16_224_in21k']:
        return_nodes = {
            'blocks.8': 'layer1',
            'blocks.9': 'layer2',
            'blocks.10': 'layer3',
    ...
    elif name in ['caformer_s18', 'caformer_s18_in21k', 'caformer_s36', 'caformer_s36_in21k']:
        return_nodes = {
            'stages.0': 'layer1',
            'stages.1': 'layer2',
            'stages.2': 'layer3',
    ...
    # print(train_nodes)
    pim_model = PluginModule(
        backbone=backbone,
        return_nodes=return_nodes,
        img_size=config.img_size,
        use_fpn=config.pim.use_fpn,
        fpn_size=config.pim.fpn_size,
        proj_type=config.pim.proj_type,
        upsample_type=config.pim.upsample_type,
        use_selection=config.pim.use_selection,
        num_classes=config.num_classes,
        num_selects=config.pim.num_selects, 
        use_combiner=config.pim.use_combiner,
        comb_proj_size=None,
    )
    return pim_model
...
if __name__ == '__main__':
    ...
    config.backbone_name = 'swin_tiny_patch4_window7_224_in21k' # 'caformer_s18' # 'swin_tiny_patch4_window7_224_in21k'
    config.pim = CN()
    config.pim.use_fpn = True
    config.pim.use_selection = True
    config.pim.use_combiner = True
    config.pim.fpn_size = 1536
    config.pim.proj_type = 'Linear'
    config.pim.upsample_type = 'Conv'
    config.pim.num_selects = CN({
        'layer1': 32,
        'layer2': 32,
        'layer3': 32,
        'layer4': 32
