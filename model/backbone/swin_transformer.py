...
def resize_pos_embed_swin(model, state_dict):
...
def swin_base_patch4_window7_224(pretrained=False, progress=True,  **kwargs):
    model_kwargs = dict(patch_size=4, window_size=7, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), **kwargs)
    return _swin('swin_base_patch4_window7_224', pretrained, progress, **model_kwargs)


@BACKBONE.register
def swin_base_patch4_window7_224_in21k(pretrained=False, progress=True, **kwargs):
    model_kwargs = dict(patch_size=4, window_size=7, embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), **kwargs)
    return _swin('swin_base_patch4_window7_224_in21k', pretrained, progress, **model_kwargs)
...
@MODEL.register
def Swin_Base_P4_W7(config):
    pretrained = config.pretrained if 'pretrained' in config else True
    return swin_base_patch4_window7_224(pretrained=pretrained, img_size=config.img_size, num_classes=config.num_classes)


@MODEL.register
def Swin_Base_P4_W7_IN21K(config):
    pretrained = config.pretrained if 'pretrained' in config else True
    return swin_base_patch4_window7_224_in21k(pretrained=pretrained, img_size=config.img_size, num_classes=config.num_classes)